...
# peft import get_peft_model, LoRAConfig, TaskType
...
class MyBartForLoraSequenceClassification(MyBartForSequenceClassification):
    def __init__(self, config: BartConfig, args, **kwargs):
        ...
        lora_config = LoraConfig(
            ...
            r=args.rank,  # rank，调整LoRA的低秩矩阵维度
            lora_alpha=alpha,  # LoRA的超参数
            target_modules=["encoder.block.0", "decoder.block.0"],  # 可以指定需要加入LoRA的层
            bias="none",  # 是否加入偏置项
            lora_dropout=0.1,
            task_type=TaskType.SEQ_CLS,
            fan_in_fan_out=True
        )
        # 使用PEFT库将LoRA模块应用到原始模型
