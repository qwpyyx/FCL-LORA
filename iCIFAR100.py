...
class iCIFAR100(CIFAR100):
    ...
    def getTrainData(self, dataset, classes):
        """
        获取当前任务的训练数据
        :param dataset: 全部训练数据集
        :param classes: 当前任务要包含的类别列表
        :return: 过滤后的训练数据集，仅包含指定类别的样本
        """
        # 过滤数据集，使其仅包含指定类别的数据
        filtered_dataset = dataset.filter(lambda example: example['label'] in classes)
    ...
    def __getitem__(self, index):
        ...
        elif self.ValidData is not None and len(self.ValidData) > 0:
